...
export class UpdateService {
  ...
  readonly results$ = merge(of(null), this.refresh$).pipe(
    ...
    switchMap(() => merge(of(null), this.load$).pipe(
      ...
      switchMap(res => iif(
        () => !this.endReached,
        this.http.get<Update[]>('/api/updates', { params: { page: res.toString(), size: this.pageSize.toString() } }),
        of([])
      )),
    ...
    return this.filteredResults$(of(filter)).pipe(
      switchMap(res => iif(() => 
        res.length >= size,
          ...
          this.http.get<Update[]>('/api/updates', { params: { page: '1', size: size.toString(), filter } }).pipe(
