...
export class MomentsComponent implements OnInit {
  ...
  onImageChange(event: any) {
    if (event.target.files.length > 0) {
      ...
      this.http.post(`${this.baseUrl}media/images`, file, { responseType: 'text' }).subscribe(
        res => {
  ...
  onSubmit() {
    if(this.form.valid && !this.submitting) {
      ...
      if(this.current.id)
        method = this.http.put(`${this.baseUrl}media/moments/${this.current.id}`, this.current);
      else
        method = this.http.post(`${this.baseUrl}media/moments`, this.current);
  ...
  onRemove() {
    if(confirm(`Are you sure want to remove this Photo?`)) {
      this.submitting = true;
      this.http.delete(`${this.baseUrl}media/moments/${this.current.id}`).subscribe(
        () => {
          this.form.resetForm();
          this.current = null;
          this.resetFile(this.image);
