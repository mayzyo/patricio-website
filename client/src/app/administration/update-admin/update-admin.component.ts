...
export class UpdateAdminComponent implements OnInit {
  ...
  onPreviewChange(event: any) {
    if (event.target.files.length > 0) {
      ...
      this.http.post(`${this.baseUrl}updates/previews`, file, { responseType: 'text' }).subscribe(
        res => {
  ...
  onSubmit() {
    if(this.form.valid && !this.submitting) {
      ...
      if(this.current.id)
        method = this.http.put(`${this.baseUrl}updates/${this.current.id}`, this.current);
      else
        method = this.http.post(`${this.baseUrl}updates`, this.current);
  ...
  onRemove() {
    if(confirm(`Are you sure want to remove this Announcement?`)) {
      this.submitting = true;
      this.http.delete(`${this.baseUrl}updates/${this.current.id}`).subscribe(
        () => {
          this.form.reset();
          this.current = null;
          this.resetFile(this.thumbnail);
