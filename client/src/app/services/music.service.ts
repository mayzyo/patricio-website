...
export class MusicService {
  ...
  readonly favourite$ = this.http.get<Music[]>(`${this.baseUrl}musics`, { params: { filter: 'FAVOURITES' } }).pipe(
    map<Music[], MusicAsync[]>(res => res.map(el => new MusicAsync(el, this))),
    shareReplay(10)
  );
...
export class MusicAsync implements Music {
  ...
  constructor(ob: Music, musics: MusicService) {
    ...
    this.audio$ = ob.audioKey && musics.contents.get(`musics/audios/${ob.audioKey}`).pipe(
